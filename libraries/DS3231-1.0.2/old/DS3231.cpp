...
byte DS3231::get24Hour(void) {
	...
	if (temp_buffer & 0b01000000) {  // h12
		hour = bcdToDec(temp_buffer & 0b00011111);
		if (temp_buffer & 0b00100000) // PM
		{
			if (hour < 12) hour += 12;
		} else
		{
...
void DS3231::setHour(byte Hour, bool def, bool formh12) {
	...
	if (h12) {
		...
		if (Hour == 0)
		{
			Hour = 0x12 | 0xb01000000;
		} else if (Hour == 12)
		{
			Hour = 0x12 | 0xb01100000;
		} else if (Hour > 12)
		{
			Hour = decToBcd(Hour-12) | 0b01100000;
		} else
		{
			Hour = decToBcd(Hour) | 0xb01000000; 
		}
	} else {
		// 24 hour
